...
(function (global) {
  ...
  function actionLink(action) {
    ...
    if (action.indexOf('protocol:') === 0) {
      ...
      return '<span class="cr-link" onclick="showProtocolDetail(\'' + pid + '\')">查看方案 ›</span>';
    }
  ...
  function renderDifferentials() {
    ...
    box.innerHTML = data.map(function (d, i) {
      var rows = d.compareItems.map(function (c) {
        return '<tr>' +
          '<td class="cr-test">' + c.test + '</td>' +
          '<td class="cr-a">' + c.a + '</td>' +
          '<td class="cr-b">' + c.b + '</td>' +
          '<td class="cr-src">' + (c.source || '') + '</td>' +
        '</tr>';
      }).join('');
      return '<div class="cr-card">' +
        '<div class="cr-card-head" onclick="crToggle(\'' + d.id + '\')">' +
          '<span class="cr-num">' + (i + 1) + '</span>' +
          '<span class="cr-title">' + d.title + '</span>' +
          '<span class="cr-toggle" data-t="' + d.id + '">▸</span>' +
        '</div>' +
        '<div class="cr-card-body" id="cr-body-' + d.id + '">' +
          '<p class="cr-context">' + d.context + '</p>' +
          '<div class="cr-table-wrap">' +
            '<table class="cr-table"><thead><tr>' +
              '<th>查体/特征</th><th class="cr-col-a">鉴别 A</th><th class="cr-col-b">鉴别 B</th><th>来源</th>' +
            '</tr></thead><tbody>' + rows + '</tbody></table>' +
          '</div>' +
          '<div class="cr-conclusion"><span class="cr-conclusion-label">结论</span>' + d.conclusion + '</div>' +
          '<div class="cr-evidence">来源：' + d.evidenceSource + '</div>' +
        '</div>' +
      '</div>';
    }).join('');
  }
    ...
    box.innerHTML = data.map(function (t, i) {
      var branches = t.branches.map(function (b, bi) {
        return '<div class="cr-branch">' +
          '<div class="cr-branch-node cr-branch-condition"><span class="cr-branch-label">若</span>' + b.condition + '</div>' +
          '<div class="cr-branch-arrow">↓</div>' +
          '<div class="cr-branch-node cr-branch-decision">' +
            '<span class="cr-branch-label">则</span>' + b.decision +
            (b.action ? actionLink(b.action) : '') +
          '</div>' +
          (b.why ? '<div class="cr-branch-why">' + b.why + '</div>' : '') +
          (b.source ? '<div class="cr-branch-src">来源：' + b.source + '</div>' : '') +
        '</div>';
      }).join('');
      return '<div class="cr-card">' +
        '<div class="cr-card-head" onclick="crToggle(\'' + t.id + '\')">' +
          '<span class="cr-num">' + (i + 1) + '</span>' +
          '<span class="cr-title">' + t.title + '</span>' +
          '<span class="cr-toggle" data-t="' + t.id + '">▸</span>' +
        '</div>' +
        '<div class="cr-card-body" id="cr-body-' + t.id + '">' +
          '<div class="cr-root"><span class="cr-branch-label">入口</span>' + t.root + '</div>' +
  ...
  function renderCases() {
    ...
    box.innerHTML = data.map(function (c, i) {
      var examRows = c.examFindings.map(function (e) {
        return '<tr>' +
          '<td class="cr-test">' + e.test + '</td>' +
          '<td>' + e.result + '</td>' +
          '<td class="cr-tag">' + (e.tag || '') + '</td>' +
        '</tr>';
      }).join('');
      var scales = (c.scaleSummary || []).map(function (s) {
        var id = (s.id || '').slice(6);
        return '<span class="cr-scale-chip" onclick="crGoScale(\'' + id + '\')">' + s.note + '</span>';
      }).join('');
      var plan = (c.plan || []).map(function (p, pi) {
        return '<li><b>' + p.step + '</b>：' + p.detail + '</li>';
      }).join('');
      var srcs = (c.sources || []).map(function (s) { return s; }).join('；');
      return '<div class="cr-card">' +
        '<div class="cr-card-head" onclick="crToggle(\'' + c.id + '\')">' +
          '<span class="cr-num">' + (i + 1) + '</span>' +
          '<span class="cr-title">' + c.title + '</span>' +
          '<span class="cr-toggle" data-t="' + c.id + '">▸</span>' +
        '</div>' +
        '<div class="cr-card-body" id="cr-body-' + c.id + '">' +
          '<div class="cr-c-label">主诉</div><p class="cr-c-text">' + c.presentation + '</p>' +
          ...
          '<div class="cr-c-label">诊断</div><p class="cr-c-text">' + c.diagnosis + '</p>' +
          '<div class="cr-c-label">方案</div><ul class="cr-plan">' + plan + '</ul>' +
          '<div class="cr-c-label">随访</div><p class="cr-c-text">' + c.followUp + '</p>' +
